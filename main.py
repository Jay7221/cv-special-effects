...
img_1 = cv2.imread('./magic_circles/ring1.png', -1)
...
def draw_line(p1, p2, size=5):
    ...
    cv2.line(img, p1, p2, (255, 255, 255), size // 2)
...
def transparent(targetImg, mask, x, y, size=None):
    if size is not None:
        targetImg = cv2.resize(targetImg, size)
        mask = cv2.resize(mask, size)

    h, w, _ = targetImg.shape
    roi = img[y:y + h, x:x + w]

    img1_bg = cv2.bitwise_and(roi.copy(), roi.copy(), mask = cv2.bitwise_not(mask))
    img2_fg = cv2.bitwise_and(targetImg, targetImg, mask = mask)
    img[y:y + h, x:x + w] = cv2.add(img1_bg, img2_fg)
...
while True:
    ...
    if result.multi_hand_landmarks:
        for hand in result.multi_hand_landmarks:
            ...
            if (THRESHOLD > ratio) and (ratio > 0.5):
            ...
            if (ratio > THRESHOLD):
                ...
                rotated1 = cv2.warpAffine(img1, M1, (wid, hei))
                rotated_mask1 = cv2.warpAffine(mask1, M1, mask1.shape)

                if diameter != 0:
                    transparent(rotated1, rotated_mask1, x1, y1, shield_size)
